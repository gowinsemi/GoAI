...
/** 
  ...
  *         @arg ERROR
  *         @arg SUCCESS
  * @brief  Initialises the UART specifying the UART Baud rate divider value.
  *         And whether the send and recieve functionality is enabled.
...
ErrorStatus UART_Init(UART_TypeDef* UARTx, UART_InitTypeDef* UART_InitStruct)
{
  ...
  /* Config baud divider */
  UARTx->BAUDDIV = SystemCoreClock / UART_InitStruct->UART_BaudRate;
